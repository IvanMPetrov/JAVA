    ...
    public static void main(String[] args) {
        ...
        while (!command.contains("Done")) {
            if (command.contains("Add Book")) {
                //ако книгата вече я има пропускаме
                //ако я няма добавяме книгата на 0 позиция
                ...
                //ако книгата я има премахваме
                //ако я няма пропускаме
                String bookName = command.split(" \\| ")[1];
                if (!isThereBook(booksArray, bookName)) {
                ...
                //ако двете книги ги има разменяме местата
                //ако ги няма пропускаме
                ...
                if (!isThereBook(booksArray, firstBook) && !isThereBook(booksArray, secondBook)) {
                    ...
                    //елемента на първа позиция го запаззвам в стринг
                    //на неговата позиция слагам втория елемент
                    //на позицията на втория елемент слагам запазения стринг
                ...
                //ако вече я има пропускаме
                //ако я няма я добавяме на последна позиция
                String bookName = command.split(" \\| ")[1];
                if (isThereBook(booksArray, bookName)) {
                ...
                //принтираме името на книгата на даденият индекс
                //ако индекса е не е в границите пропускаме
                int index = Integer.parseInt(command.split(" \\| ")[1]);
                if (index >= 0 && index < booksArray.size()) {
                    //искам да взема текста от индекса и да го принтирам
